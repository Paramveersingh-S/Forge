...
def scan_adapter(adapter_path: str | Path, threshold: float = DOMINANCE_THRESHOLD) -> bool:
    ...
    if suspicious_layers:
        console.print(
            f"[bold red]✗ SECURITY WARNING: {len(suspicious_layers)} layers exhibit rank-1 dominance![/bold red]"
        )
        console.print(
            "[red]This is a strong indicator of a targeted backdoor or data poisoning.[/red]"
        )
