...
Works with unpaired preference data — you only need per-example
labels (desirable / undesirable) rather than matched pairs. This
makes data collection much easier than DPO.

Reference: Ethayarajh et al., "KTO: Model Alignment as Prospect
Theoretic Optimization" (2024).
"""
...
class KTOTrainer:
    ...
    Supports:
    - Unpaired preference data (label per example, not pairs)
    ...
    def validate_config(self, config: ForgeConfig) -> None:
        """Validate KTO-specific config requirements."""
        if config.training.method != "kto":
            raise ValueError(f"KTOTrainer requires method='kto', got '{config.training.method}'")

    def train(self, config: ForgeConfig, resume: bool = False) -> None:
        """Run KTO training."""
        self.validate_config(config)

        console.print("[bold]Starting KTO training...[/bold]")
        ...
        # Load unpaired preference dataset
        ...
        # Train
        console.print("\n[bold green]🔥 KTO training started![/bold green]")
